...
def joinFactors(factors):
    ...
    "*** YOUR CODE HERE ***"
    unconditionedVariables, conditionedVariables = set(), set()
    variableDomainsDict = None
    for factor in factors:
        for var in factor.unconditionedVariables():
            conditionedVariables.discard(var)
    ...
    joined = Factor(list(unconditionedVariables), list(conditionedVariables), variableDomainsDict)
    for assignment in joined.getAllPossibleAssignmentDicts():
        probability = 1
        for factor in factors:
            probability *= factor.getProbability(assignment)
        joined.setProbability(assignment, probability)
    return joined
    "*** END YOUR CODE HERE ***"
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        eliminated = Factor(unconditionedVariables, factor.conditionedVariables(), factor.variableDomainsDict())
        print(factor)
        for assignment in eliminated.getAllPossibleAssignmentDicts():
            probability = 0
            for val in factor.variableDomainsDict()[eliminationVariable]:
                assignment[eliminationVariable] = val
                probability += factor.getProbability(assignment)
            eliminated.setProbability(assignment, probability)
        return eliminated
        "*** END YOUR CODE HERE ***"
...
def normalize(factor):
    ...
    normalized = Factor(list(unconditioned), list(conditioned), factor.variableDomainsDict())
    normalizationConstant = 0
    for assignment in normalized.getAllPossibleAssignmentDicts():
        normalizationConstant += factor.getProbability(assignment)
    if normalizationConstant == 0:
        return None
    for assignment in normalized.getAllPossibleAssignmentDicts():
        normalized.setProbability(assignment, factor.getProbability(assignment) / normalizationConstant)
    return normalized
    "*** END YOUR CODE HERE ***"
